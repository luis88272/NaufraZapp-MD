...
const isCommand1 = /^(gimageee|imagennnn?)$/i.test(command)
const isCommand2 = /^(plakky|plnnay2)$/i.test(command)
//const isCommand3 = /^(play2)$/i.test(command)
const isCommand4 = /^(fgmpjj3|dlmpjn3|getannud|yjkt(a|mp3)?)$/i.test(command)
const isCommand5 = /^(ytmp3docjj|ytadocjjj)$/i.test(command)
const isCommand6 = /^(fgmp4jj|dlmpjj4|getvidjj|ytjj(v|mp4)?)$/i.test(command)
const isCommand7 = /^(ytmp4djjoc|ytvdonnc|playjj4|playjj3)$/i.test(command)
const isCommand8 = /^(facebnnnook|fb|facejnbookdl|fbnndl)$/i.test(command)
const isCommand9 = /^(mebndiafire(dl)?|dlmediafinnre)$/i.test(command)
const isCommand10 = /^(ytmaxxx)$/i.test(command)
const isCommand11 = /^(tkdl|tiktokkk)$/i.test(command)
const isCommand12 = /^(ytmaxdoccc)$/i.test(command)
const isCommand13 = /^(dalle3|aiimg|aimage|iaimagen|openaimage|openaiimage)$/i.test(command)
const isCommand14 = /^(openjourney|journey|midjourney)$/i.test(command)
const isCommand15 = /^(spotijjfy|musjjic)$/i.test(command)
const isCommand16 = /^(spkkot(ify)?searbbch)$/i.test(command)
const isCommand17 = /^(ijj(nsta)?g(ram)?(dl)?|igijjmage|ijjgdownload)$/i.test(command)
const isCommand18 = /^((dl)?tbbw(it(ter(dl|x)?)?)?|xjj|t?txjj)$/i.test(command)
const isCommand19 = /^(ghgitclone|clonjjarepo|clonjjarrepo|repjjoclonar)$/i.test(command)
const isCommand20 = /^(bardimg|bardimage|geminiimg|geminiimage|geminimg|geminimage)$/i.test(command)

async function reportError(e) {
let errb = await m.reply('Ocurrio un error inesperado.')
await console.log(`Ocurrio un error inesperado.`)
await console.log(e)
...
await conn.sendFile(m.chat, await (await fetch(urlDALLE)).buffer(), 'image.jpg', `> Aqui tienes tu imagen.\n\n_${text}_`, m)
} catch {
try{   
let response = await fetch(`https://api.lolhuman.xyz/api/diffusion-prompt?apikey=${lolkeysapi}&prompt=${text}`)
let image = await response.buffer()
await conn.sendFile(m.chat, image, 'image.jpg', `> Aqui tienes tu imagen.\n\n_${text}_`, m)
} catch {
try{
let res = `https://api.lolhuman.xyz/api/dall-e?apikey=${lolkeysapi}&text=${text}`  
await conn.sendFile(m.chat, res, 'image.jpg', `> Aqui tienes tu imagen.\n\n_${text}_`, m)
} catch (e) {
reportError(e)} 
}}      
break
...
text = m.quoted.text
} else return conn.reply(m.chat, `> *Ingrese un texto para convertirlo en imagen.*`, m)
m.reply(wait)
try {
await Draw(text).then((img) => {
conn.sendFile(m.chat, img, 'image.jpg', `> Aqui tienes tu imagen.\n\n_${text}_`, m)
})
} catch (e) {
reportError(e)}        
break
...
handler.command = /^(dalle3|aiimg|aimage|iaimagen|openaimage|openaiimage|openjourney|journey|midjourney|bardimg|bardimage|geminiimg|geminiimage|geminimg|geminimage)$/i
